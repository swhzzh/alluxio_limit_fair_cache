...
public class BlockInStream extends InputStream implements BoundedStream, Seekable,
  ...
  public static BlockInStream create(FileSystemContext context, BlockInfo info,
    ...
    if (sourceIsLocal && shortCircuit && !sourceSupportsDomainSocket) {
      ...
      try {
        BlockInStream blockInStream = createLocalBlockInStream(context, dataSource, blockId, blockSize, options);
  ...
  private static BlockInStream createLocalBlockInStream(FileSystemContext context,
    ...
    LocalFileDataReader.Factory factory = new LocalFileDataReader.Factory(context, address, blockId, chunkSize, options);
    if (factory .isPathEmpty()){
      return null;
    }
    return new BlockInStream(
        factory, address, BlockInStreamSource.LOCAL, blockId, length);
  }
...
/*  private String tryShortCircuitRead(FileSystemContext context, long blockId, boolean isPromote, WorkerNetAddress address){
    final GrpcBlockingStream<OpenLocalBlockRequest, OpenLocalBlockResponse> mStream;
    OpenLocalBlockRequest request = OpenLocalBlockRequest.newBuilder()
        .setBlockId(blockId).setPromote(isPromote).build();
    AlluxioConfiguration conf = context.getClusterConf();
    int readBufferSize = conf.getInt(PropertyKey.USER_NETWORK_READER_BUFFER_SIZE_MESSAGES);
    long dataTimeoutMs = conf.getMs(PropertyKey.USER_NETWORK_DATA_TIMEOUT_MS);
    try {
      BlockWorkerClient blockWorkerClient = context.acquireBlockWorkerClient(address);
      mStream = new GrpcBlockingStream<>(blockWorkerClient::openLocalBlock, readBufferSize,
          MoreObjects.toStringHelper(LocalFileDataReader.class)
              .add("request", request)
              .add("address", address)
              .toString());
      mStream.send(request, dataTimeoutMs);
      OpenLocalBlockResponse response = mStream.receive(dataTimeoutMs);
