...
public class BlockWorkerImpl extends BlockWorkerGrpc.BlockWorkerImplBase {
  ...
  public BlockWorkerImpl(WorkerProcess workerProcess, FileSystemContext fsContext,
    ...
    //mUserNameToUserIdMap.put("swh", 0L);
  }
  ...
  @Override
  public StreamObserver<ReadRequest> readUserBlock(StreamObserver<ReadResponse> responseObserver) {
    CallStreamObserver<ReadResponse> callStreamObserver =
        (CallStreamObserver<ReadResponse>) responseObserver;
    if (ZERO_COPY_ENABLED) {
      callStreamObserver =
  ...
  @Override
  public StreamObserver<WriteRequest> writeUserBlock(
      StreamObserver<WriteResponse> responseObserver) {
    ServerCallStreamObserver<WriteResponse> serverResponseObserver =
        (ServerCallStreamObserver<WriteResponse>) responseObserver;
    if (ZERO_COPY_ENABLED) {
...
/*    ShortCircuitBlockReadHandler handler = new ShortCircuitBlockReadHandler(
  ...
  @Override
  public StreamObserver<OpenLocalBlockRequest> openUserLocalBlock(
      StreamObserver<OpenLocalBlockResponse> responseObserver) {
    AuthenticatedUserInfo userInfo = getAuthenticatedUserInfo();
    Long userId = mUserNameToUserIdMap.get(userInfo.getAuthorizedUserName());
    if (userId == null){
  ...
  @Override
  public StreamObserver<CreateLocalBlockRequest> createUserLocalBlock(
      StreamObserver<CreateLocalBlockResponse> responseObserver) {
    AuthenticatedUserInfo userInfo = getAuthenticatedUserInfo();
    Long userId = mUserNameToUserIdMap.get(userInfo.getAuthorizedUserName());
    if (userId == null){
  ...
  @Override
  public void asyncUserCache(AsyncCacheRequest request,
      StreamObserver<AsyncCacheResponse> responseObserver) {
    AuthenticatedUserInfo userInfo = getAuthenticatedUserInfo();
    Long userId = mUserNameToUserIdMap.get(userInfo.getAuthorizedUserName());
    if (userId == null){
  ...
  @Override
  public void removeUserBlock(RemoveBlockRequest request,
      StreamObserver<RemoveBlockResponse> responseObserver) {
    AuthenticatedUserInfo userInfo = getAuthenticatedUserInfo();
    Long userId = mUserNameToUserIdMap.get(userInfo.getAuthorizedUserName());
    if (userId == null){
  ...
  public void moveBlock(MoveBlockRequest request,
    ...
    LOG.info("user {} tries to moveBlock", userInfo.getAuthorizedUserName());
    long sessionId = IdUtils.createSessionId();
    Long finalUserId = userId;
    RpcUtils.call(LOG, (RpcUtils.RpcCallableThrowsIOException<MoveBlockResponse>) () -> {
      mWorkerProcess.getWorker(BlockWorker.class).moveUserBlockToMedium(sessionId, finalUserId,
...
/*    long sessionId = IdUtils.createSessionId();
  ...
  @Override
  public void moveUserBlock(MoveBlockRequest request,
      StreamObserver<MoveBlockResponse> responseObserver) {
    AuthenticatedUserInfo userInfo = getAuthenticatedUserInfo();
    Long userId = mUserNameToUserIdMap.get(userInfo.getAuthorizedUserName());
    if (userId == null){
