...
public abstract class AbstractBlockMeta {
   ...
   * @return
   */
/*  public static String userTempPath(StorageDir dir, long sessionId, long userId, long blockId){
    final String tmpDir = ServerConfiguration.get(PropertyKey.WORKER_DATA_TMP_FOLDER);
    final int subDirMax = ServerConfiguration.getInt(PropertyKey.WORKER_DATA_TMP_SUBDIR_MAX);

    return PathUtils.concatPath(dir.getUserDirPath(userId), tmpDir, sessionId % subDirMax,
        String.format("%x-%x", sessionId, blockId));
  }*/
...
/*  public static String userCommitPath(StorageDir dir, long userId, long blockId){
    return PathUtils.concatPath(dir.getUserDirPath(userId), blockId);
  }*/
