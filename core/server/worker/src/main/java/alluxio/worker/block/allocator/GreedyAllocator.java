...
public final class GreedyAllocator implements Allocator {
  ...
  public StorageDirView allocateUserBlockWithView(long sessionId, long userId, long blockSize,
    ...
    mMetadataView = Preconditions.checkNotNull(view, "BlockMetadataView");
    return allocateUserBlock(sessionId, userId, blockSize, location);
  }

  private StorageDirView allocateUserBlock(long sessionId, long userId, long blockSize, BlockStoreLocation location){
    Preconditions.checkNotNull(location, "location");
    if (location.equals(BlockStoreLocation.anyTier())){
      for (StorageTierView tierView : mMetadataView.getTierViews()) {
        for (StorageDirView dirView : tierView.getDirViews()) {
    ...
    if (!mediumType.equals(BlockStoreLocation.ANY_MEDIUM)
      ...
      for (StorageTierView tierView : mMetadataView.getTierViews()) {
        for (StorageDirView dirView : tierView.getDirViews()) {
          if (dirView.getMediumType().equals(mediumType)
              && dirView.getUserAvailableBytes(userId) >= blockSize){
            return dirView;
          }
        }
      }
