...
public class AsyncCacheRequestManager {
    ...
    try {
      mAsyncCacheExecutor.submit(() -> {
        ...
        try {
          ...
          // 客户端时, 首先对本地worker缓存是否命中进行了判断, 命中就短路读, 如果到达这里说明是进行rpc远程读
          if (isSourceLocal) {
            // 未命中worker缓存, 委托本地worker从ufs缓存
            ASYNC_CACHE_UFS_BLOCKS.inc();
            result = cacheBlockFromUfs(blockId, blockLength, openUfsBlockOptions);
          } else {
            // 命中远程worker缓存, 委托本地worker从远程worker读取并缓存
   ...
   * @param request the async cache request fields will be available
   */
  public void submitUserRequest(AsyncCacheRequest request, long userId) {
    ASYNC_CACHE_REQUESTS.inc();
    long blockId = request.getBlockId();
    long blockLength = request.getLength();
    if (mPendingRequests.putIfAbsent(blockId, request) != null) {
    ...
    try {
      mAsyncCacheExecutor.submit(() -> {
        ...
        try {
          // Check if the block has already been cached on this worker
          long lockId = mBlockWorker.lockUserBlockNoException(Sessions.ASYNC_CACHE_SESSION_ID, userId, blockId);
          if (lockId != BlockLockManager.INVALID_LOCK_ID) {
            try {
              mBlockWorker.unlockUserBlock(lockId);
            } catch (BlockDoesNotExistException e) {
              LOG.error("Failed to unlock block on async caching. We should never reach here", e);
            }
            ASYNC_CACHE_DUPLICATE_REQUESTS.inc();
          ...
          if (isSourceLocal) {
            // 未命中worker缓存, 委托本地worker从ufs缓存
            ASYNC_CACHE_UFS_BLOCKS.inc();
            result = cacheUserBlockFromUfs(userId, blockId, blockLength, openUfsBlockOptions);
          } else {
            // 命中远程worker缓存, 委托本地worker从远程worker读取并缓存
            ASYNC_CACHE_REMOTE_BLOCKS.inc();
            InetSocketAddress sourceAddress =
                new InetSocketAddress(request.getSourceHost(), request.getSourcePort());
            result = cacheUserBlockFromRemoteWorker(userId,
                blockId, blockLength, sourceAddress, openUfsBlockOptions);
          }
          LOG.debug("Result of {} async caching block {}: {}", userId, blockId, result);
        } catch (Exception e) {
          LOG.warn("Async cache request failed.\n{}\nError: {}", request, e);
        } finally {
          if (result) {
  ...
  private boolean cacheUserBlockFromUfs(long userId, long blockId, long blockSize, Protocol.OpenUfsBlockOptions openUfsBlockOptions){
    try {
      if (!mBlockWorker.openUserUfsBlock(Sessions.ASYNC_CACHE_SESSION_ID, userId, blockId, openUfsBlockOptions)){
    ...
    try (BlockReader reader = mBlockWorker
        .readUserUfsBlock(Sessions.ASYNC_CACHE_SESSION_ID, userId, blockId, 0)) {
      ...
      try {
        mBlockWorker.closeUserUfsBlock(Sessions.ASYNC_CACHE_SESSION_ID, userId, blockId);
  ...
  private boolean cacheBlockFromRemoteWorker(long blockId, long blockSize,
    ...
    // need to decide whether insert userId for worker-worker remote read
  ...
  private boolean cacheUserBlockFromRemoteWorker(long userId, long blockId, long blockSize,
      InetSocketAddress sourceAddress, Protocol.OpenUfsBlockOptions openUfsBlockOptions) {
    try {
      mBlockWorker.createUserBlockRemote(Sessions.ASYNC_CACHE_SESSION_ID, userId, blockId,
          mStorageTierAssoc.getAlias(0), "", blockSize);
    } catch (BlockAlreadyExistsException e) {
      // It is already cached
      return true;
    ...
    try (BlockReader reader =
         ...
         BlockWriter writer =
             mBlockWorker.getTempBlockWriterRemote(Sessions.ASYNC_CACHE_SESSION_ID, blockId)) {
      BufferUtils.fastCopy(reader.getChannel(), writer.getChannel());
      mBlockWorker.commitUserBlock(Sessions.ASYNC_CACHE_SESSION_ID, blockId, false);
      return true;
    } catch (AlluxioException | IOException e) {
      LOG.warn("Failed to async cache block {} from remote worker ({}) on copying the block: {}",
          blockId, sourceAddress, e.getMessage());
      try {
        mBlockWorker.abortUserBlock(Sessions.ASYNC_CACHE_SESSION_ID, blockId);
      } catch (AlluxioException | IOException ee) {
        LOG.warn("Failed to abort block {}: {}", blockId, ee.getMessage());
      }
      return false;
