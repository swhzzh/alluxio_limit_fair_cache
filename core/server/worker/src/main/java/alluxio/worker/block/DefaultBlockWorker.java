...
public final class DefaultBlockWorker extends AbstractWorker implements BlockWorker {
  ...
  DefaultBlockWorker(BlockMasterClientPool blockMasterClientPool,
    ...
    super(Executors
        .newFixedThreadPool(6, ThreadFactoryUtils.build("block-worker-heartbeat-%d", true)));
  ...
  public void start(WorkerNetAddress address) throws IOException {
    ...
    getExecutorService().submit(
        ...
        (int) FormatUtils.parseTimeSize("10s"), ServerConfiguration.global(), ServerUserState.global()));
  ...
  public Set<Long> getAllUsers() {
    return mBlockStore.getAllUsers();
  }
  ...
  public void abortUserBlock(long sessionId, long blockId)
      ...
      IOException {
    mBlockStore.abortUserBlock(sessionId, blockId);
  }
  ...
  public void commitUserBlock(long sessionId, long blockId, boolean pinOnCreate)
    ...
    long userId = -1;
    try {
      TempBlockMeta tempBlockMeta = mBlockStore.getTempBlockMeta(sessionId, blockId);
      if (tempBlockMeta != null){
        userId = tempBlockMeta.getUserId();
        mBlockStore.commitUserBlock(sessionId, blockId, pinOnCreate);
      }
    } catch (BlockAlreadyExistsException e) {
      LOG.debug("Block {} has been in block store, this could be a retry due to master-side RPC "
          + "failure, therefore ignore the exception", blockId, e);
    ...
    BlockMasterClient blockMasterClient = mBlockMasterClientPool.acquire();
    try {
      BlockMeta meta = mBlockStore.getUserBlockMeta(sessionId, userId, blockId, lockId);
      BlockStoreLocation loc = meta.getBlockLocation();
      String mediumType = loc.mediumType();
      long length = meta.getBlockSize();
      // TODO(swh): 2020/3/2 add user specific block store meta
      BlockStoreMeta storeMeta = mBlockStore.getBlockStoreMeta();
      Long bytesUsedOnTier = storeMeta.getUsedBytesOnTiers().get(loc.tierAlias());
      blockMasterClient.commitBlock(mWorkerId.get(), bytesUsedOnTier, loc.tierAlias(), mediumType,
          blockId, length);
  ...
  public BlockMeta getUserBlockMeta(long sessionId, long userId, long blockId, long lockId)
      throws BlockDoesNotExistException, InvalidWorkerStateException {
    return mBlockStore.getUserBlockMeta(sessionId, userId, blockId, blockId);
  }
  ...
  @Override
  public void moveUserBlock(long sessionId, long userId, long blockId, String tierAlias)
      throws BlockDoesNotExistException, BlockAlreadyExistsException, InvalidWorkerStateException,
      WorkerOutOfSpaceException, IOException {
    // Because the move operation is expensive, we first check if the operation is necessary
    BlockStoreLocation dst = BlockStoreLocation.anyDirInTier(tierAlias);
    ...
    // Execute the block move if necessary
    mBlockStore.moveUserBlock(sessionId, userId, blockId, dst);
  }
  ...
  @Override
  public void moveUserBlockToMedium(long sessionId, long userId, long blockId, String mediumType)
      throws BlockDoesNotExistException, BlockAlreadyExistsException, InvalidWorkerStateException,
      WorkerOutOfSpaceException, IOException {
    BlockStoreLocation dst = BlockStoreLocation.anyDirInTierWithMedium(mediumType);
    long lockId = mBlockStore.lockUserBlock(sessionId, userId, blockId);
  ...
  @Override
  public void readSharedBlock(long sessionId, long userId, long blockId, long lockId)
  ...
  public String readUserBlock(long sessionId, long userId, long blockId, long lockId)
      throws BlockDoesNotExistException, InvalidWorkerStateException, UserOutOfSpaceException {
    BlockMeta blockMeta = mBlockStore.getBlockMeta(sessionId, blockId, lockId);
    if (mBlockStore.hasUserBlockMeta(userId, blockId)){
  ...
  public void requestUserSpace(long sessionId, long userId, long blockId, long additionalBytes)
      throws BlockDoesNotExistException, WorkerOutOfSpaceException, IOException {
    mBlockStore.requestUserSpace(sessionId, blockId, additionalBytes);
  }
  ...
  @Override
  public void updatePinList(Map<Long, Set<Long>> userIdToPinnedInodes) {
    mBlockStore.updateUserPinnedInodes(userIdToPinnedInodes);
  }
  ...
  public void closeUserUfsBlock(long sessionId, long userId, long blockId)
    ...
    try {
      ...
      if (mBlockStore.getTempBlockMeta(sessionId, blockId) != null) {
        try {
          commitUserBlock(sessionId, blockId, false);
        } catch (BlockDoesNotExistException e) {
          // This can only happen if the session is expired. Ignore this exception if that happens.
          LOG.warn("Block {} does not exist while being committed.", blockId);
        } catch (InvalidWorkerStateException e) {
  ...
  public void cleanupSessionAndUser(long sessionId) {
    mBlockStore.cleanupSessionAndUser(sessionId);
    mUnderFileSystemBlockStore.cleanupSession(sessionId);
  }
    ...
    public void heartbeat() {
      try {
        if (mBlockStore.checkStorage()) {
          ...
          mSpaceReserver.updateAllUserStorageInfo();
        }
      } catch (Exception e) {
