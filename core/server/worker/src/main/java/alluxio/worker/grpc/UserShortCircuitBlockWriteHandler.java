 ...
 */
@NotThreadSafe
class UserShortCircuitBlockWriteHandler implements StreamObserver<CreateLocalBlockRequest> {
   ...
   */
  UserShortCircuitBlockWriteHandler(BlockWorker blockWorker,
      StreamObserver<CreateLocalBlockResponse> responseObserver, AuthenticatedUserInfo userInfo, long userId) {
    ...
    RpcUtils.streamingRPCAndLog(LOG, new RpcUtils.StreamingRpcCallable<CreateLocalBlockResponse>() {
      ...
      public CreateLocalBlockResponse call() throws Exception {
        if (request.getOnlyReserveSpace()) {
          mBlockWorker.requestUserSpace(mSessionId, mUserId, request.getBlockId(), request.getSpaceToReserve());
          ...
          if (mSessionId == INVALID_SESSION_ID) {
            ...
            String path = mBlockWorker.createUserBlock(mSessionId, mUserId, request.getBlockId(),
      ...
      public void exceptionCaught(Throwable throwable) {
        if (mSessionId != INVALID_SESSION_ID) {
          ...
          mBlockWorker.cleanupSessionAndUser(mSessionId);
  ...
  public void handleBlockCompleteRequest(boolean isCanceled) {
    ...
    RpcUtils.streamingRPCAndLog(LOG, new RpcUtils.StreamingRpcCallable<CreateLocalBlockResponse>() {
          ...
          public CreateLocalBlockResponse call() throws Exception {
            ...
            try {
              if (isCanceled) {
                mBlockWorker.abortUserBlock(mSessionId, mRequest.getBlockId());
              } else {
                mBlockWorker.commitUserBlock(mSessionId, mRequest.getBlockId(), mRequest.getPinOnCreate());
              }
            } finally {
