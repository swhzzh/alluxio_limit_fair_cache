...
public final class UnderFileSystemBlockReader implements BlockReader {
  ...
  private void cancelBlockWriter() throws IOException {
    ...
    try {
      mBlockWriter.close();
      mBlockWriter = null;
      if (mBlockMeta.getUserId() == -1L){
        mLocalBlockStore.abortBlock(mBlockMeta.getSessionId(), mBlockMeta.getBlockId());
      }
      else {
        mLocalBlockStore.abortUserBlock(mBlockMeta.getSessionId(), mBlockMeta.getBlockId());
      }
    ...
    try {
      if (mBlockWriter == null && offset == 0 && !mBlockMeta.isNoCache()) {
        ...
        if (userId == -1L){
          ...
          mLocalBlockStore.createUserBlock(mBlockMeta.getSessionId(), userId, mBlockMeta.getBlockId(), loc,
              mInitialBlockSize);
        }
