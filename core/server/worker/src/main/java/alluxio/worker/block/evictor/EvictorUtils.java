...
public final class EvictorUtils {
  ...
  @Nullable
  public static StorageDirView selectUserDirWithRequestedSpace(long userId, long bytesToBeAvailable,
      BlockStoreLocation location, BlockMetadataEvictorView mManagerView) {
    if (location.equals(BlockStoreLocation.anyTier())) {
      for (StorageTierView tierView : mManagerView.getTierViews()) {
        for (StorageDirView dirView : tierView.getDirViews()) {
          if (dirView.getUserAvailableBytes(userId) >= bytesToBeAvailable) {
            return dirView;
          }
        }
      }
    ...
    StorageDirView dirView = tierView.getDirView(location.dir());
    return (dirView.getUserAvailableBytes(userId) >= bytesToBeAvailable) ? dirView : null;
  }
