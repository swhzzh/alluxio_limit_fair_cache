...
public class StorageDirEvictorView extends StorageDirView {
  ...
  public long getUserAvailableBytes(long userId) {
    if (!mUserIdToBlockIdsToMoveOutMap.containsKey(userId)){
      // TODO: 2020/2/29 throw user doesn't exist exception
      return 0;
    }
    return mDir.getUserAvailableBytes(userId) +
        mUserIdToBlocksToMoveOutSizeMap.get(userId) - mUserIdToBlocksToMoveInSizeMap.get(userId);
  }
  ...
  public long getUserEvictableBytes(long userId){
    return mDir.getUserBlocks(userId).stream()
        .filter(blockMeta -> mMetadataView.isUserBlockEvictable(userId, blockMeta.getBlockId()))
        .mapToLong(BlockMeta::getBlockSize).sum();
  }
  ...
  public void clearUserBlockMarks(long userId){
    mUserIdToBlockIdsToMoveOutMap.remove(userId);
    mUserIdToBlockIdsToMoveInMap.remove(userId);
    mUserIdToBlocksToMoveOutSizeMap.remove(userId);
    mUserIdToBlocksToMoveInSizeMap.remove(userId);
  }
