 ...
 */
public class UserDelegationWriteHandler implements StreamObserver<alluxio.grpc.WriteRequest> {
   ...
   */
  public UserDelegationWriteHandler(WorkerProcess workerProcess,
  ...
  private AbstractWriteHandler createWriterHandler(alluxio.grpc.WriteRequest request) {
    switch (request.getCommand().getType()) {
      case ALLUXIO_BLOCK:
        return new UserBlockWriteHandler(mWorkerProcess.getWorker(BlockWorker.class), mResponseObserver,
            mUserInfo, mDomainSocketEnabled, mUserId);
      case UFS_FILE:
        return new UfsFileWriteHandler(mWorkerProcess.getUfsManager(), mResponseObserver,
            mUserInfo);
      case UFS_FALLBACK_BLOCK:
        return new UserUfsFallbackBlockWriteHandler(mWorkerProcess.getWorker(BlockWorker.class),
            mWorkerProcess.getUfsManager(), mResponseObserver, mUserInfo, mDomainSocketEnabled, mUserId);
      default:
